...
function PhotoField({ question, error, formState, setFormState, files, setFiles }) {
  ...
  function handleChange(evt) {
    ...
    if (evt.target.files.length > 0)
      setFiles(Array.from(evt.target.files).map((f) => ({ tag: question.tag, file: f })))
  }
  ...
  function RenderFiles() {
    ...
    return (
      <div className='rtl:space-x-reverse space-x-4 mt-4'>
